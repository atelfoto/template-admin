(function(factory) {
    ...
    function convertAttribute(node, state) {
        ...
        return {
            name: isBoolean || implied ?
                name.slice(implied ? 1 : 0, isBoolean ? -1 : void 0) : name,
            value,
            boolean: isBoolean,
            implied,
            valueType
    ...
    function consumeValue(scanner) {
        ...
        return result.length ? { type: 'CSSValue', value: result } :
            void 0;
    }
    ...
    function isFragmentDelimiter(token) {
        return isArgumentDelimiter(token) || isWhiteSpace$2(token);
    }
    ...
    function narrowToNonSpace(editor, range) {
        ...
        while (startOffset < endOffset && isSpace$1(text[startOffset])) {
            startOffset++;
        }
        while (endOffset > startOffset && isSpace$1(text[endOffset - 1])) {
            endOffset--;
        }
    ...
    function isQuotedString(str) {
        return str.length > 1 && isQuote$2(str[0]) && str[0] === str.slice(-1);
    }
        ...
        if (!stack.length && scanner.pos !== pos) {
            ...
            return {
                ...
                start: options.prefix ?
                    start - options.prefix.length : pos - abbreviation.length,
                end: pos
            };
        }
    }
    ...
    function consumePaired(scanner) {
        return eatPair(scanner, 60 /* LeftAngle */ , 62 /* RightAngle */ , opt$1) ||
            eatPair(scanner, 40 /* LeftRound */ , 41 /* RightRound */ , opt$1) ||
            eatPair(scanner, 91 /* LeftSquare */ , 93 /* RightSquare */ , opt$1) ||
            eatPair(scanner, 123 /* LeftCurly */ , 125 /* RightCurly */ , opt$1);
    }
    /**
    ...
    function attributeValue(scanner) {
        ...
        return eatQuoted(scanner, opt$1) || consumePaired(scanner) || unquoted(scanner);
    }
    /**
    ...
    function processingInstruction(scanner) {
        const start = scanner.pos;
        if (consumeArray$1(scanner, piStart) && ident(scanner)) {
            const name = scanner.current();
            while (!scanner.eof()) {
                if (consumeArray$1(scanner, piEnd)) {
                    break;
                }
        ...
        scan$1(source, (type, start, end, delimiter) => {
            if (type === "selector" /* Selector */ ) {
                ...
                if (left && left[0] < pos && end > pos) {
                    ...
                    inner && push$1(result, inner);
                    push$1(result, [left[0], end]);
                }
                ...
                if (property && property[0] < pos && Math.max(delimiter, end) > pos) {
                    ...
                    push$1(result, [start, end]);
                    push$1(result, [property[0], delimiter !== -1 ? delimiter + 1 : end]);
                }
            }
            ...
            if (type === "blockEnd" /* BlockEnd */ ) {
                ...
                if (pendingProperty) {
                    if (pendingProperty.start <= pos && end >= pos) {
                        ...
                        push$1(result, [pendingProperty.start, delimiter + 1]);
                        push$1(result, [start, end]);
                        releasePending();
                        return false;
                    }
    ...
    function extract(text, pos = text.length, options) {
        const opt = Object.assign(Object.assign({}, defaultOptions$3), options);
     ...
     */
    function getAbbreviationContext(editor, pos) {
        ...
        if (syntax) {
            ...
            if (context) {
                return Object.assign(Object.assign({}, context), { options: getOutputOptions(editor, pos, context.inline) });
            }
        }
    }
    /**
    ...
    function getCSSContext(code, pos) {
        let section = 0;
        let valid = true;
        let name = '';
        scan$1(code, (type, start, end) => {
            ...
            if (start <= pos && end >= pos) {
                ...
                valid = type === "propertyValue" /* PropertyValue */ || type === "propertyName" /* PropertyName */ ;
                return false;
            }
            switch (type) {
    ...
    function createHTMLAbbreviationContext(code, tag) {
        const attrs = {};
     ...
     */
    function inlineCSSContext(code, pos) {
        let name = '';
        scan$1(code, (type, start, end, delimiter) => {
            if (end >= pos) {
                return false;
    ...
    function getSyntaxForStyleTag(code, tag) {
        for (const attr of attributes(code.slice(tag.start, tag.end), tag.name)) {
     ...
     */
    function extract$1(code, pos, hostSyntax = 'html', options) {
        ...
        if (ctx) {
            const abbrData = extractAbbreviation(code, pos, Object.assign({ lookAhead: !isCSS(ctx.syntax), type: isCSS(ctx.syntax) ? 'stylesheet' : 'markup' }, options));
            if (abbrData) {
     ...
     */
    function getOptions(editor, pos, withContext = false) {
    ...
    class AbbreviationTracker {
        constructor(start, pos, length, forced = false) {
                this.forced = forced;
    ...
    function startTracking(editor, start, pos, params) {
        const tracker = new AbbreviationTracker(start, pos, editor.getValue().length, params === null || params === void 0 ? void 0 : params.forced);
        if (params) {
     ...
     */
    function stopTracking(editor, skipRemove) {
    ...
    function handleSelectionChange(editor, caret = getCaret(editor)) {
        const tracker = getTracker(editor);
        if (tracker) {
    ...
    function extractTracker(editor, pos) {
        ...
        const abbr = extract$1(getContent(editor), pos, syntax, { prefix });
        if (abbr) {
     ...
     */
    function shouldStopTracking(tracker, pos) {
        if (tracker.forced) {
            ...
            token(stream, state) {
                ...
                if (token.type === 'Quote') {
                    ...
                    if (token.open) {
                        ...
                        state[token.context]--;
                        const lastBrace = last$5(state.braces);
                ...
                if (stream.eol() && state.braces.length && !state.parseError) {
                    const pos = last$5(state.braces).start;
    ...
    function getTokenName(token, state) {
        const prev = last$5(state.tokens);
        switch (token.type) {
            case 'Bracket':
                return `bracket`;
            case 'Field':
    ...
    function expandAbbreviation$1(editor, tabKey) {
        ...
        const abbr = extract$1(line, pos.ch, syntax);
        if (abbr) {
    ...
    function removeComment(editor, { range, commentStart, commentEnd }) {
        ...
        if (commentStart && text.startsWith(commentStart)) {
            ...
            if (isSpace$1(text[startOffset])) {
                startOffset += 1;
            }
            if (endOffset && isSpace$1(text[text.length - endOffset - 1])) {
                endOffset += 1;
            }
    ...
    function getHTMLBlockRange(source, pos, xml = false) {
        ...
        scan(source, (name, type, start, end) => {
            ...
            if (type === 1 /* Open */ ) {
                // Allocate tag object from pool
                stack.push(allocTag$2(pool, name, start, end));
            } else if (type === 3 /* SelfClose */ ) {
                if (start < pos && pos < end) {
                    // Matched given self-closing tag
    ...
    function allocTag$2(pool, name, start, end) {
        if (pool.length) {
            const tag = pool.pop();
            tag.name = name;
            tag.start = start;
    ...
    function releaseTag$2(pool, tag) {
        pool.push(tag);
    }
    ...
    function findNewEditPoint(editor, pos, inc) {
        ...
        while (curPos < docSize && curPos >= 0) {
            ...
            if (isQuote$2(cur) && next === cur && prev === '=') {
            ...
            if (!rangeEmpty(innerRange)) {
                ...
                if (start.line !== end.line) {
                    ...
                    while (line <= end.line) {
                        ...
                        if (isSpace$1(substr(editor, indentRange))) {
            ...
            for (const sel of selections) {
                ...
                if (tag) {
                    ...
                    if (close) {
                        ...
                        let closing = isSpace$1(getChar(editor, open[1] - 2)) ? '/' : ' /';
                        ...
                        if (getChar(editor, open[1] - 2) === '/') {
                            let start = open[1] - 2;
                            let end = open[1] - 1;
                            if (isSpace$1(getChar(editor, start - 1))) {
                                start--;
                            }
    ...
    function registerEmmetExtension(CM) {
        ...
        CM.defineExtension('emmetOptions', function(pos = 0, withContext) {
